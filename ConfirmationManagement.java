...
public class ConfirmationManagement extends JFrame {
    ...
    private void editConfirmation() {
        ...
        if (selectedRow == -1) {
            JOptionPane.showMessageDialog(null, "Please select a confirmation to edit.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        // Show dialog to edit a confirmation
        JTextField confirmationTypeField = new JTextField(model.getValueAt(selectedRow, 2).toString());
        JTextField confirmationDateField = new JTextField(model.getValueAt(selectedRow, 3).toString());
        ...
        int result = JOptionPane.showConfirmDialog(null, panel, "Edit Confirmation", JOptionPane.OK_CANCEL_OPTION);
        if (result == JOptionPane.OK_OPTION) {
            try {
                pstmt = conn.prepareStatement("UPDATE confirmations SET borrowing_id=?, confirmation_type=?, confirmation_date=? WHERE confirmation_id=?");
                pstmt.setInt(1, Integer.parseInt((String) borrowingComboBox.getSelectedItem()));
                pstmt.setString(2, confirmationTypeField.getText());
                pstmt.setDate(3, java.sql.Date.valueOf(confirmationDateField.getText()));
    ...
    private void deleteConfirmation() {
        ...
        int selectedRow = table.getSelectedRow();
        if (selectedRow == -1) {
            JOptionPane.showMessageDialog(null, "Please select a confirmation to delete.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        // Confirm deletion
        int option = JOptionPane.showConfirmDialog(null, "Are you sure you want to delete this confirmation?", "Confirmation", JOptionPane.YES_NO_OPTION);
        if (option == JOptionPane.YES_OPTION) {
            try {
                pstmt = conn.prepareStatement("DELETE FROM confirmations WHERE confirmation_id=?");
